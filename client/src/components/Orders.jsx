...
const YourOrders = () => {
  ...
  const { myOrders = [], loading = false, error = null } = useSelector((state) => state.order || {});
  ...
  const getFilteredOrders = () => {
    ...
    switch (timeFilter) {
      ...
      case 'past-6-months':
        ...
        break;
      case '2024':
        return myOrders.filter(order => new Date(order.createdAt).getFullYear() === 2024);
      case '2023':
        return myOrders.filter(order => new Date(order.createdAt).getFullYear() === 2023);
      case '2022':
        return myOrders.filter(order => new Date(order.createdAt).getFullYear() === 2022);
      default:
        return myOrders;
    }
  ...
  const filteredOrders = getSearchedOrders(getTabFilteredOrders(getFilteredOrders()));
      ...
      {error && (
        <div className="text-center py-12">
          <p className="text-red-600">
            Error loading orders: {typeof error === 'string' ? error : error.message || 'An error occurred'}
          </p>
        </div>
      )}
            ...
            <div key={order._id} className="border border-gray-300 rounded-lg overflow-hidden">
              ...
              <div className="bg-gray-100 px-6 py-3 grid grid-cols-4 gap-4 text-sm">
                ...
                <div className="text-right">
                  <div className="text-gray-600 text-xs mb-1">ORDER # {order._id?.slice(-8)}</div>
              ...
              {/* Order Items */}
              <div className="p-6">
                <div className={`font-bold text-lg mb-4 ${getStatusColor(order.status || 'pending')}`}>
